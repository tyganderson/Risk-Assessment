    ...
    render() {
        ...
        const AllData = () => {
            ...
            const deleteEntry = async (key) => {
                if(window.confirm("Do you wish to delete this entry?") == true) {
                    await remove(key);
            ...
            return (
                ...
                data.slice(0).reverse().map(item => (
                    ...
                    <div>
                        <div key={item.header.tripNum} className='detail-block'>
                        ...
                        <button className='admin-block' onClick={() => deleteEntry(item.header.tripNum)}>
                            <p><FaRegTrashAlt size={20} color="white"/></p>
                        </button>
                    </div> // end div key
                )) // end data.map loop
            ) // end return
